...
if __name__ == "__main__":
    ...
    # solve with linear interpolated P-iLQR
    ...
    total = diff_time + fp_time + bp_time
    if cfg.run_ddp:
        print("Linear Interpolated DDP took : ", np.sum(total), "[sec]")
    else:
    ...
    # crearte gif
    ts = np.linspace(0.0, max(comp_time[-1], comp_time_linear[-1]), 50)
    xs = []
    us = []
    interpolated_xs = []
    interpolated_us = []
